...
function CataloguePage({ navigate, params }) {
  ...
  useEffect(() => { if (params?.category) setSelectedCat(params.category); }, [params?.category]);
...
function FilterItem({ label, count, active, onClick }) {
  return (
    <button onClick={onClick}
      style={{ background: active ? 'var(--navy)' : 'transparent', border: 'none', padding: '6px 10px', borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'background 0.15s', textAlign: 'left' }}
